...
impl InvocationLease {
    ...
    pub(crate) fn guard(&self) -> InvocationLeaseGuard {
...
pub(crate) struct InvocationLeaseGuard(InvocationLease);
...
pub enum BrokerError {
    ...
    #[error("LLM error: {0}")]
    Llm(#[from] langchart_adapters::llm::LlmError),

    #[error("MCP error: {0}")]
    Mcp(#[from] langchart_adapters::mcp::McpError),

    #[error("memory error: {0}")]
    Memory(#[from] langchart_adapters::memory::MemoryError),

    #[error("artifact error: {0}")]
    Artifact(#[from] langchart_adapters::artifact::ArtifactError),

    #[error("secrets error: {0}")]
    Secrets(#[from] langchart_adapters::secrets::SecretsError),

    #[error("event sink error: {0}")]
    EventSink(#[from] langchart_adapters::event::EventSinkError),
...
impl CapabilityBroker {
    pub fn new(
        ...
        Self {
            ...
            event_sink,
            artifact_store: None,
            envelope_authority: Arc::new(()),
        }
    }

    pub(crate) fn authorize_envelope(&self, envelope: &mut CapabilityEnvelope) -> InvocationLease {
    ...
    pub async fn memory_store(
        &self,
        envelope: &CapabilityEnvelope,
        record: langchart_adapters::memory::MemoryRecord,
    ) -> Result<langchart_adapters::memory::MemoryId, BrokerError> {
        let permit = self.begin_operation(envelope)?;
...
mod tests {
    ...
    async fn server_budget_and_credentials_are_enforced_inside_broker() {
        ...
        let _lease = broker.authorize_envelope(&mut envelope);
    ...
    async fn resource_reads_require_broker_authority_and_matching_policy() {
        ...
        let _lease = broker.authorize_envelope(&mut authorized);
        broker
    ...
    async fn artifact_operations_require_explicit_permissions_and_use_bound_run_id() {
        ...
        let _lease = broker.authorize_envelope(&mut denied);
        let error = broker
        ...
        let _lease = broker.authorize_envelope(&mut allowed);
        broker
    ...
    async fn revoked_invocation_lease_rejects_broker_calls() {
        ...
        let lease = broker.authorize_envelope(&mut envelope);
        lease.revoke();
        ...
        let error = broker
            .memory_search(
                &envelope,
                MemoryQuery {
                    scope: langchart_adapters::memory::MemoryScope::Global,
                    mode: langchart_adapters::memory::QueryMode::Keyword {
                        text: "test".into(),
                    },
                    limit: 1,
                    min_score: None,
